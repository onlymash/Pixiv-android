...
public class FragmentSpecialCollec extends BaseFragment {
    ...
    private void getSpecialCollection() {
        mProgressBar.setVisibility(View.VISIBLE);
        Call<SpecialArticalResponse> call = new RestClient()
                .getRetrofit_AppAPI()
                .create(AppApiPixivService.class)
                .getSpecialArtical(Common.getLocalDataSet().getString("Authorization", ""), 0);
        call.enqueue(new retrofit2.Callback<SpecialArticalResponse>() {
            @Override
            public void onResponse(Call<SpecialArticalResponse> call, retrofit2.Response<SpecialArticalResponse> response) {
                if (getView() != null) {
                    if (response.body().getNext_url() != null) {

                        Common.showLog(response.body().getNext_url());
                    }
                }
            }

            @Override
            public void onFailure(Call<SpecialArticalResponse> call, Throwable throwable) {
